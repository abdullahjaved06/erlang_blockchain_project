-module(main).
-export([start/0, demo_part1/1, demo_election/1]).
...
%% Demo for Part II: standalone election among V validators
demo_election(ValidatorCount) ->
    io:format("== Part II election demo with ~p validators ==~n", [ValidatorCount]),

    ValidatorIds = lists:seq(1, ValidatorCount),

    %% Initial proposer group at bootstrap: first 10% of validators
    GroupSize =
        case ValidatorCount div 10 of
            0 -> 1;
            N -> N
        end,
    ProposerGroup = lists:sublist(ValidatorIds, GroupSize),

    io:format("Bootstrap proposer group: ~p~n", [ProposerGroup]),

    %% Spawn validators (they register themselves as validator_1, validator_2, ...)
    [validator:start(Id, ValidatorIds, ProposerGroup) || Id <- ValidatorIds],
    io:format("Spawned ~p validators.~n", [ValidatorCount]),

    %% Let the first validator in the proposer group be the initial head
    validator:start_election(hd(ProposerGroup)).
